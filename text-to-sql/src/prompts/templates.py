...
## 注意事项
- 如果用户没有指定聚合方式，默认使用 sum（营养）或 count（视频）
...
### 查询今日总热量
SELECT total_calorie FROM daily_record WHERE date = CURRENT_DATE

### 查询本周蛋白质摄入总和
SELECT SUM(total_protein) FROM daily_record WHERE date >= CURRENT_DATE - INTERVAL '7 days'

### 查询某餐的营养素（需要计算重量）
SELECT SUM(f.protein * m.weight_g / 100.0) as total_protein
FROM meal_record m
JOIN food f ON m.food_id = f.id
WHERE m.meal_type = '早餐' AND m.daily_id IN (
    SELECT id FROM daily_record WHERE date = CURRENT_DATE
)
...
WHERE up_name = '某UP主' AND publish_date >= CURRENT_DATE - INTERVAL '7 days'
...
- 对于"最近"类查询，使用 ORDER BY + LIMIT 或 INTERVAL
